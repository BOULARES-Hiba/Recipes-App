...
const Details = () => {
  ...
  const { recipe, setRecipe, changeFavorites, favorites } =
    ...
    <>
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className=" flex flex-col">
            <div>
              <button
                onClick={() => changeFavorites(recipe.recipe)}
                className="py-2 px-4 rounded-lg text-sm uppercase font-medium mt-2  shadow-md bg-pink-400 text-white text-nowrap"
              >
            {favorites && favorites.length > 0 && favorites.findIndex(
              (item) => item.id === recipe?.recipe?.id
            ) !== -1
